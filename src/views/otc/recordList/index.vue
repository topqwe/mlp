      ...
      <van-pull-refresh style="min-height: 100vh;" v-model="refreshing" @refresh="onRefresh">
        ...
        >
          ...
          <div class=""  v-for="(item,index) in listDatas" :key="index">
            <div class="liebiao">
              <div class="pinglie">
                <div class="l1">{{ $t('价格') }}</div>
                <div class="r1" >{{ item.prize }} {{ item.orderPriceType }}</div>
<!--                <img @click="copyData(item.order_no)" src="../../../assets/image/public/copy.png" style="width: 18px;height: 18px;margin-left: 4px;" />-->
              </div>
              <div class="pinglie">
                <div class="l1">{{ $t('数量') }}</div>
                <div class="r1">{{ item.prize }}</div>
              </div>
              <div class="pinglie">
                <div class="l1">{{ $t('总计') }} </div>
                <div class="r1">{{ item.id }}{{item.orderPriceType}}</div>
              </div>
              <div class="pinglie">
                <div class="l1">{{ $t('状态') }}</div>
                <div class="r1 chenggong" v-if="item.id==0" >{{ $t('成功') }}</div>
<!--                <div class="r1 " >{{ $t('成功') }}</div>-->
                <div class="r1 shibai" v-else >{{ $t('失败') }}</div>
              </div>
...
<style>
.otcRecord .van-hairline--bottom::after{
